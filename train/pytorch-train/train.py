...
parser.add_argument('--workers', type=int, help='number of data loading workers', default=2)  # 4
parser.add_argument('--batchSize', type=int, default=256, help='input batch size')  # 128
parser.add_argument('--imgH', type=int, default=32, help='the height of the input image to network')
parser.add_argument('--imgW', type=int, default=280, help='the width of the input image to network')  # 256
parser.add_argument('--nh', type=int, default=256, help='size of the lstm hidden state')
parser.add_argument('--niter', type=int, default=3, help='number of epochs to train for')
parser.add_argument('--lr', type=float, default=0.0005, help='learning rate for Critic, default=0.00005')  # 0.005
...
parser.add_argument('--experiment', help='Where to store samples and models',default='./models')
...
test_dataset = dataset.lmdbDataset(
    root=opt.valroot, transform=dataset.resizeNormalize((280, 32)))  # (256, 32)
...
def val(net, criterion, max_iter=3):
    ...
    for i in range(max_iter):
        ...
        #torch.ctcLoss need logSoftMax as input
        preds = preds.log_softmax(2)
        #preds = F.log_softmax(preds,dim=0)
